...
export function googleAuthStart() {
  if (!GOOGLE_ENABLED) {
    return (_req, res) => res.status(503).json({ error: 'Google login not configured' });
  }
...
export function googleAuthCallback() {
  if (!GOOGLE_ENABLED) {
    return (_req, res) => res.redirect(`${CLIENT_ORIGIN}/login?error=google_not_configured`);
  }
  return (req, res, next) => {
    passport.authenticate('google', { session: false }, (err, data) => {
      if (err || !data || !data.token) return res.redirect(`${CLIENT_ORIGIN}/login?error=google_auth_failed`);
